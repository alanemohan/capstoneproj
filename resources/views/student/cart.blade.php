...
<div class="space-y-6" x-data="{ payModal: false }">
    ...
    <nav class="flex items-center gap-2 text-sm text-gray-500">
        <a href="{{ route('student.courses') }}" class="hover:text-indigo-600 transition">{{ __('messages.courses') }}</a>
        <span class="text-gray-300">›</span>
        <span class="text-gray-800 font-medium">{{ __('messages.cart') }}</span>
    </nav>

    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-800">🛒 {{ __('messages.cart') }}
            @if($cartItems->isNotEmpty())
                <span class="text-base font-normal text-gray-400 ml-2">({{ $cartItems->count() }} {{ Str::plural('course', $cartItems->count()) }})</span>
            @endif
        </h1>
        @if($cartItems->isNotEmpty())
            <a href="{{ route('student.courses') }}" class="text-sm text-indigo-600 hover:underline">+ {{ __('Add more courses') }}</a>
        @endif
    </div>
    ...
    @if($cartItems->isEmpty())
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-16 text-center">
            ...
            <h2 class="text-xl font-bold text-gray-700 mb-2">{{ __('Your cart is empty') }}</h2>
            <p class="text-gray-400 text-sm mb-6">{{ __('Browse our catalog and add courses you would like to learn.') }}</p>
