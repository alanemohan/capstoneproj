                ...
                <div class="p-5 space-y-4">
                    ...
                    @elseif($course->isFree())
                        <form method="POST" action="{{ route('student.courses.enroll', $course) }}" x-data="{ submitting: false }" @submit="submitting = true">
                            ...
                            <button type="submit"
                                    :disabled="submitting"
                                    class="w-full bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 disabled:cursor-not-allowed text-white py-3 rounded-xl font-bold transition shadow-sm shadow-emerald-200 flex items-center justify-center gap-2">
                    ...
                    @else
                        ...
                        @if($inCart)
                            ...
                            <form method="POST" action="{{ route('student.cart.remove', $course) }}" x-data="{ submitting: false }" @submit="submitting = true">
                                ...
                                <button type="submit"
                                        :disabled="submitting"
                                        class="w-full text-sm text-gray-500 hover:text-red-600 py-2 transition disabled:opacity-60 disabled:cursor-not-allowed">
                        ...
                        @else
                            <form method="POST" action="{{ route('student.cart.add', $course) }}" x-data="{ submitting: false }" @submit="submitting = true">
                                ...
                                <button type="submit"
                                        :disabled="submitting"
                                        class="w-full bg-amber-500 hover:bg-amber-600 disabled:opacity-60 disabled:cursor-not-allowed text-white py-3 rounded-xl font-bold transition shadow-sm shadow-amber-200 flex items-center justify-center gap-2">
                                    <svg x-show="submitting" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                                    </svg>
