    ...
    <div class="flex flex-col bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden" style="height: calc(100vh - 160px); min-height: 500px;">
        ...
        <div class="bg-gradient-to-r from-purple-600 to-indigo-600 text-white p-4 flex-shrink-0">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-bold">{{ __('AI Study Assistant') }}</h2>
                    <p class="text-xs text-purple-200">{{ __('Powered by rule-based NLP | Available 24/7') }}</p>
                </div>
                <div class="ml-auto flex items-center gap-1.5">
                    <span class="w-2 h-2 bg-green-400 rounded-full animate-pulse"></span>
                    <span class="text-xs text-purple-200">{{ __('Online') }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="px-4 py-3 bg-purple-50 border-b border-purple-100 flex-shrink-0">
            <p class="text-xs text-purple-600 font-medium mb-2">{{ __('Quick Topics:') }}</p>
        ...
        <div id="chat-messages" class="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div class="flex items-start gap-3">
                ...
                <div class="bg-purple-50 rounded-2xl rounded-tl-none p-3 max-w-sm">
                    ...
                    <p class="text-xs text-gray-400 mt-2">{{ __('Just now') }}</p>
                </div>
            </div>
        ...
        <div class="p-4 border-t border-gray-200 flex-shrink-0 bg-white">
            ...
            <form id="chat-form" class="flex gap-3">
                ...
                <input type="text" id="chat-input"
                       placeholder="{{ __('Ask me anything about your syllabus...') }}"
            ...
            <p class="text-xs text-gray-400 mt-2 text-center">{{ __('Press Enter to send. This AI covers Classes 6-10 syllabus topics.') }}</p>
        </div>
    </div>
</div>
...
function appendMessage(message, isUser) {
    const time = new Date().toLocaleTimeString('{{ app()->getLocale() }}', { hour: '2-digit', minute: '2-digit' });
...
chatForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        typingIndicator.classList.add('hidden');
        appendMessage({ response: "{{ __('Sorry, I could not connect right now. Please check your internet connection.') }}", subject: null }, false);
    }
});
