...
<div class="space-y-6">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">✏️ {{ __('messages.quizzes') }}</h1>
        <p class="text-gray-500 text-sm mt-1">{{ __('Test your knowledge and track your progress') }}</p>
    </div>
    ...
    <form method="GET" data-no-loading class="flex gap-3">
        ...
        <button type="submit" class="px-5 py-2.5 bg-primary-600 text-white rounded-xl hover:bg-primary-700 transition text-sm font-medium">{{ __('Filter') }}</button>
        @if(request('subject'))
            <a href="{{ route('student.quizzes') }}" class="px-5 py-2.5 border border-gray-300 text-gray-600 rounded-xl hover:bg-gray-50 transition text-sm">{{ __('Clear') }}</a>
        @endif
    </form>
    ...
    @if($quizzes->isEmpty())
        <div class="bg-white rounded-xl p-12 text-center shadow-sm border border-gray-100">
            ...
            <p class="text-gray-500">{{ __('No quizzes available yet. Check back soon!') }}</p>
        </div>
    @else
                        ...
                        @elseif($canAttempt)
                            <a href="{{ route('student.quiz.start', $quiz->id) }}"
                               class="block w-full text-center bg-primary-600 hover:bg-primary-700 text-white py-2.5 rounded-xl text-sm font-semibold transition">
                                {{ $bestAttempt ? '🔄 ' . __('Retake Quiz') : '▶ ' . __('Start Quiz') }}
                            </a>
                        @else
