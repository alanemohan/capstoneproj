...
@section('title', 'Edit Quiz - Nabha Learning')
...
<div class="max-w-4xl mx-auto" x-data="quizBuilder()">
    ...
    <form method="POST" action="{{ route('teacher.quizzes.update', $quiz->id) }}" class="space-y-5">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="class_level" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm">
                        ...
                        @foreach(['Class 6','Class 7','Class 8','Class 9','Class 10'] as $class)
                            <option value="{{ $class }}" {{ old('class_level', $quiz->class_level) === $class ? 'selected':'' }}>{{ $class }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" name="time_limit" value="{{ old('time_limit', $quiz->time_limit) }}" required min="5" max="180"
                           class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Passing Marks (%) *</label>
                    <input type="number" name="passing_marks" value="{{ old('passing_marks', $quiz->passing_marks) }}" required min="0" max="100"
                           class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Max Attempts *</label>
                    <input type="number" name="max_attempts" value="{{ old('max_attempts', $quiz->max_attempts) }}" required min="1" max="10"
                           class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm">
                </div>
            </div>
        </div>
                    ...
                    <div class="border border-gray-200 rounded-xl p-4 bg-gray-50">
                        ...
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <input type="number" :name="`questions[${index}][marks]`" x-model="q.marks" min="1"
                        ...
                        <!-- Explanation -->
                        <input type="text" :name="`questions[${index}][explanation]`" x-model="q.explanation"
...
function quizBuilder() {
    return {
        questions: {!! json_encode($quiz->questions->map(function($q, $i) {
            return [
                'key' => $i + 1,
                'id' => $q->id,
        ...
        nextKey: {{ $quiz->questions->count() + 1 }},
        addQuestion() {
            this.questions.push({ key: this.nextKey++, id: null, text: '', type: 'mcq', correct: 'a', marks: 1, option_a: '', option_b: '', option_c: '', option_d: '', explanation: '' });
        },
