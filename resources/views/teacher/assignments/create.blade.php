...
@section('title', 'Create Assignment - Nabha Learning')
...
<div class="max-w-2xl mx-auto animate-fade-in">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-xl font-bold text-gray-900 tracking-tight" style="font-family: var(--font-display);">Create New Assignment</h1>
    </div>
    ...
    <form method="POST" action="{{ route('teacher.assignments.store') }}" class="bg-white rounded-xl border border-gray-200 p-6 space-y-5 shadow-sm">
        @csrf
        ...
        <div>
            ...
            <textarea name="description" rows="5" required
                      ...
                      placeholder="Enter detailed instructions, questions, or guidelines for the students...">{{ old('description') }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" name="max_marks" value="{{ old('max_marks', 100) }}" required min="1"
                       class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500/40 focus:border-emerald-500/30 text-xs font-medium text-gray-808"
                       placeholder="e.g., 100">
            </div>
        </div>
