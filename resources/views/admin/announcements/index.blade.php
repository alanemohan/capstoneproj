...
<div class="space-y-6" x-data="{ 
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">{{ __('messages.notice_board') }}</h1>
            <p class="text-gray-500 text-sm mt-1">{{ __('messages.manage') }} global system announcements</p>
        </div>
        ...
        <table class="w-full text-sm text-left">
            <thead class="bg-gray-50 text-gray-500 uppercase text-xs font-semibold">
                <tr>
                    ...
                    <th class="px-6 py-4">{{ __('messages.benefits_en') }}</th> {{-- Reusing generic content label if possible, or just Content --}}
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($announcements as $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 truncate max-w-xs">{{ Str::limit(strip_tags($item->getLocalized('content')), 50) }}</td>
                        ...
                        <td class="px-6 py-4 flex items-center justify-end gap-3">
                            ...
                            <form method="POST" action="{{ route('admin.announcements.destroy', $item->id) }}" onsubmit="return confirm('{{ __('messages.confirm_action') }}')">
                                @csrf @method('DELETE')
                ...
                @empty
                    <tr><td colspan="4" class="px-6 py-8 text-center text-gray-500">{{ __('messages.no_announcements') }}</td></tr>
                @endforelse
            </tbody>
        </table>
        </div>
    ...
    <div x-show="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" x-cloak>
        <div class="bg-white rounded-2xl w-full max-w-lg p-6 mx-4 relative" @click.away="showModal = false">
            ...
            <h2 class="text-xl font-bold mb-4" x-text="editMode ? '{{ __('messages.edit') }} Notice' : '{{ __('messages.submit') }} Notice'"></h2>
            ...
            <form method="POST" :action="editMode ? `/admin/announcements/${form.id}` : '{{ route('admin.announcements.store') }}'" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('messages.title') }} (EN)</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('messages.target_audience') }}</label>
