...
class QuizAttempt extends Model
{
    ...
    {
        $quizPassingMarks = $this->relationLoaded('quiz') && $this->quiz ? (int) $this->quiz->passing_marks : null;

        if ($quizPassingMarks !== null) {
            return (float) $this->percentage >= $quizPassingMarks;
        }
    ...
    public function calculateAndSave(array $submittedAnswers, Quiz $quiz): void
    {
        ...
        $passed = $percentage >= (int) $quiz->passing_marks;
        ...
        $this->update([
            ...
            'passed' => $passed,
            'completed_at' => now(),
            'time_taken' => max(0, now()->diffInSeconds($this->started_at)),
        ]);
    }
}
